	...
	# inner loop by hits stored in MAXPERC hash
        ...
        foreach my $perc ( sort keys %SEENPERC ) {
        	foreach my $hit (  sort  keys %{ $SEENPERC{$perc} } ) {
				...
				$detline="$samplename\t$ampliconname\t$hit\t$numreads\t$numhits\t$min_percIdent\t$max_percIdent\t$avg_percIdent\t$min_aln\t$max_aln\t$avg_aln\t$min_cov\t$max_cov\t$avg_cov\t$percIdentity[$percentileIndex]";
