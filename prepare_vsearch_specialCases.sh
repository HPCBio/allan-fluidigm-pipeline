#!/bin/bash
# program:     vsearch_specialCases.sh
# author:      Gloria Rendon
# date:        April, 2017
# description: to launch vsearch for samples and barcodes specified in the configuration file and to produce summaries of the results by SAMPLE
#########################################################################################
...
set +x;
echo -e "\n\n##############################################################################" >&2;
echo -e "\n############   STARTS vsearch_specialCases.sh                       #########" >&2;
echo -e "\n##############################################################################\n\n" >&2; 
set -x;

if [ $# -lt 1 ]
then
     echo -e "Program $0 to launch vsearch for samples in SAMPLEFILE and barcodes in BARCODEFILE \n";
     echo -e "SAMPLEFILE and BARCODEFILE are specified in the configuration file\n\n"
...
barcodefile=$( cat $runfile | grep -w BARCODEFILE | cut -d '=' -f2 )
...
queue=$( cat $runfile | grep -w QUEUE_VSEARCH | cut -d '=' -f2 )
...
default_maxhits_noV=10
default_maxhits_V=10
default_mincov=50
...
then 
    echo -e "$0 stopped at line $LINENO\sREASON=$barcodefile file is empty "
    exit 1;
fi
...
then
    ...
    echo -e "resetting output folder $outputdir... let us do the bulk of this cleanup work at the sample level instead";
    #`rm -rf $outputdir`
    #`mkdir -p $outputdir`
    rm $outputdir/BARCODE_names.txt
    rm $outputdir/SAMPLE_names.txt
    rm $outputdir/database.tsv
    rm $outputdir/database.fa
fi
...
cp $barcodefile  $outputdir/BARCODE_names.txt
...
if [ ! -s $outputdir/BARCODE_names.txt ]
then
    echo -e "$0 stopped at line $LINENO\sREASON=$outputdir/BARCODE_names.txt file not copied"
    exit 1;
fi
...
barcodefile=$outputdir/BARCODE_names.txt                 # local copy of config file
...
do
	...
	then
	    ...
	    if [[ $sample =~  /^V/ ]]
	    then
		 maxhits=$default_maxhits_V
	    else    
		 maxhits=$default_maxhits_noV
	    fi
	fi
	...
	ln -s $barcodefile BARCODE_names.txt
	ln -s $xrefdb database.tsv

	set +x;
	echo -e "\n\n#####################################################################\n" >&2;
	echo -e "############    START LOOP2 OVER BARCODES                           #####\n" >&2;
	...
	do
		...
		set +x; echo -e "\n############ processing sample=${sample} and barcode=$barcode  ###############\n\n" >&2; set -x;

		prefix=${sample}-${barcode}
		query_file=${prefix}_SearchReady.fasta
		inputfile=$inputdir/$sample/$prefix/$query_file	            
		...
		set +x; echo -e "\n############ done processing sample=${sample} and barcode=$barcode Next one please ###############\n\n" >&2; set -x;
	...
	echo -e "perl $scriptdir/vsearch_calcSummary_specialCases.pl  $outdir $sample $outputfile $maxhits $percentile" >> $qsub1

	if [ $cleanup == "YES" ]
	then
		echo -e "if [ -s $outputfile ]"  >> $qsub1
		echo -e "then"  >> $qsub1		        
		echo -e "    unlink BARCODE_names.txt" >> $qsub1
...
echo -e "\n\n##############################################################################" >&2;
echo -e "\n############   ENDS vsearch_specialCases.sh                            #########" >&2;
echo -e "\n##############################################################################\n\n" >&2; 
set -x;
